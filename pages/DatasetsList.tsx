...
const DatasetsList: React.FC<DatasetsListProps> = ({ navigate }) => {
  ...
  const filteredDatasets = useMemo(() => {
    return datasets.filter(ds => {
      ...
      const categoryMatch = categoryFilter === 'All' || linkedDataTypes.some(dt => dt.category === categoryFilter);
      const dataTypeMatch = dataTypeFilter === 'All' || linkedDataTypes.some(dt => dt.id === dataTypeFilter);
  ...
  const { sortedData, handleSort, sortColumn, sortDirection } = useSort<Dataset>(filteredDatasets, 'name');

  const SortableHeader: React.FC<{ column: keyof Dataset; children: React.ReactNode }> = ({ column, children }) => (
    <th 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="block w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white"
              >
                <option value="All">All DataTypes</option>
          ...
          <table className="w-full text-left">
            <thead className="bg-neutral-50 border-b border-neutral-200">
              <tr>
                ...
                <SortableHeader column="source_organization">Source Organization</SortableHeader>
                <SortableHeader column="source_type">Type</SortableHeader>
